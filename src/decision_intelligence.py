...
def recommended_action(risk_level: str, uncertainty: float) -> str:
    if risk_level == "CRITICAL":
        return "Immediate Human Review / Isolate or Contain"
    if risk_level == "HIGH":
        return "Escalate and Investigate"
    if risk_level == "MODERATE":
        return "Investigate and Monitor"
    if uncertainty >= 0.70:
        return "Monitor / Human Review for Uncertainty"
    return "Monitor"
...
def generate_decision_outputs(
    predictions: np.ndarray,
    risk_df: pd.DataFrame,
    important_features: Optional[pd.Series] = None,
) -> pd.DataFrame:
    out = risk_df.copy()
    ...
    out["human_review_required"] = (
        out["risk_level"].isin(["HIGH", "CRITICAL"])
        | (out["uncertainty"] >= 0.70)
    )
    out["recommended_action"] = [
        recommended_action(risk, uncertainty)
        for risk, uncertainty in zip(out["risk_level"], out["uncertainty"])
    ]

    if important_features is not None:
        values = important_features.astype(str).tolist()
        if len(values) < len(out):
            values += ["Unavailable"] * (len(out) - len(values))
    ...
    out["decision_reason"] = (
        ...
        + out["risk_level"]
        + "; attack_probability="
        + out["attack_probability"].round(3).astype(str)
        + "; uncertainty="
        + out["uncertainty"].round(3).astype(str)
    )
    return out
